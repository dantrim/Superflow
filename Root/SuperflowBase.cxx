...
void SuperflowBase::determine_output_filename(TString input_sample)
{
    ...
    if (m_runMode == SuperflowRunMode::data) {
        ...
        bool data18 = input_sample.Contains("data18_13TeV");
        bool is_data = (data15 || data16 || data17 || data18);

        if(is_data) {
            TString data_run; data_run.Form("%d",nt.evt()->run);
            string year = (data15 ? "2015" : data16 ? "2016" : data17 ? "2017" : data18 ? "2018" : "");
            if(year=="") {
                ...
                cout << app_name << "SuperflowBase::determine_output_filename    (seen as: 2015? " << data15 << ", 2016? " << data16 << ", 2017? " << data17 << ", 2018? " << data18 << ")" << endl;
                exit(1);
            }
            ...
            cout << app_name << "SuperflowBase::determine_output_filename    ERROR    It does not contain either the 'data15_13TeV', 'data16_13TeV', 'data17_13TeV', or 'data18_13TeV' grouping and the run mode is" << endl;
...
void SuperflowBase::initialize_output_files(TString input)
{
    ...
    for (int i = 0; i < (int)m_varType.size(); i++) { //m_varFloatArray[i] = 1.0; }
        vector<double> null_float(25, -999);
        vector<int> null_int(25, -999);
        vector<bool> f(25, false);
        m_varFloatArray.push_back(null_float);
...
SuperflowBase& SuperflowBase::operator<<(std::function<vector<int>(Superlink*, var_int_array*)> var_)
{
    if (m_varState == SupervarState::open && m_sysState == SupersysState::closed && !m_superVar_hasFunction) {
        ...
        m_varExprBoolArray.push_back(m_nullExprBoolArray);
        m_varExprFloatArray.push_back(m_nullExprFloatArray);
        m_varExprIntArray.push_back(var_); // Fill
        m_varExprInt.push_back(m_nullExprInt);
        m_varExprBool.push_back(m_nullExprBool);
        m_varExprVoid.push_back(m_nullExprVoid);
